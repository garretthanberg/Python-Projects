...
# Importing the other modules so phonebook_func
...
def create_db(self):
    ...
    with conn:
        ...
        # Always commit() to save changes & close the database connection.
        conn.commit()
    conn.close()
    first_run(self)
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and(len(var_email) > 0): # This enforces the user to provide both names.
        conn = sqlite3.connect('db_phonebook.db')
        with conn:
            cursor = conn.cursor()
